...
describe('QuerySerializer', () => {
    test('nested', () => {
        ...
        const output = QuerySerializer.nested(input);
        expect(output).toBe(
    ...
    test('nested with special characters', () => {
        ...
        const output = QuerySerializer.nested(input);
        expect(output).toBe(
            'user.name=John%20Doe&user.email=john.doe%40example.com&user.password=P%40%24%24w0rd!'
        );
    });

    test('parse with special characters', () => {
        const input =
            'user.name=John%20Doe&user.email=john.doe%40example.com&user.password=P%40%24%24w0rd%21';
